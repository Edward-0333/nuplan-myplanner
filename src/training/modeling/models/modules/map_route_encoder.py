...
class MapRouteEncoder(nn.Module):
    def __init__(
        self,
        polygon_channel=6,
        dim=128,
    ...
    def forward(self, data) -> torch.Tensor:
        polygon_center = data["route_map"]["polygon_center"]
        polygon_type = data["route_map"]["polygon_type"].long()
        polygon_has_speed_limit = data["route_map"]["polygon_has_speed_limit"]
        polygon_speed_limit = data["route_map"]["polygon_speed_limit"]
        point_position = data["route_map"]["point_position"]
        point_vector = data["route_map"]["point_vector"]
        point_orientation = data["route_map"]["point_orientation"]
        valid_mask = data["route_map"]["valid_mask"]
        # test1 = valid_mask[0].cpu().numpy()
        # test2 = valid_mask[1].cpu().numpy()
        ...
        x_polygon += x_type + x_speed_limit
