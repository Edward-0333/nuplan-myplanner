...
class TTFeature(AbstractModelFeature):
    ...
    @classmethod
    def collate(cls, feature_list: List[TTFeature]) -> TTFeature:
        batch_data = {}
        ...
        if "route_map" in feature_list[0].data:
            pad_keys.append("route_map")
        ...
        return TTFeature(data=batch_data)

    def to_feature_tensor(self) -> TTFeature:
        new_data = {}
    ...
    def to_numpy(self) -> TTFeature:
        ...
        if self.data_n is not None:
            ...
            new_data_n = None
        return TTFeature(data=new_data, data_p=new_data_p, data_n=new_data_n)

    def to_device(self, device: torch.device) -> TTFeature:
        new_data = {}
        for k, v in self.data.items():
            new_data[k] = to_device(v, device)
        return TTFeature(data=new_data)

    def serialize(self) -> Dict[str, Any]:
        return {"data": self.data}

    @classmethod
    def deserialize(cls, data: Dict[str, Any]) -> TTFeature:
        return TTFeature(data=data["data"])
